...
const port  = "1414", landing = 'index.html', source ='';
...
function serverController(hostname) {
    const server = http.createServer((request, response) => {
        ...
        if (request.url == '/') {
            var file = path.join(__dirname, source, landing);       // delivers index.html by default
            sendFile(file, landing, 'text/html', request, response);
        } else {
            var file = path.join(__dirname, source, request.url);   // delivers requested resource
